...
# ----------------------------
# User Choice: Manual Input or CSV Upload
# ----------------------------
st.subheader("Choose Input Method")
option = st.radio("How do you want to input data?", ("Manual Input", "Upload CSV"))
...
if option == "Manual Input":
    ...
    if uploaded_file is not None:
        ...
        if model is None:
            ...
            probas = model.predict_proba(data)[:, 1] * 100 if hasattr(model, "predict_proba") else np.repeat(50.0, len(data))

            results = data.copy()
            results['Prediction'] = predictions
            results['Probability'] = probas
            results['Risk'] = np.where(predictions == 1, "High Risk 💔", "Low Risk 💚")

            st.write("Predictions:", results)

            csv = results.to_csv(index=False).encode('utf-8')
            st.download_button("Download Results as CSV", csv, "heart_results.csv", "text/csv")
